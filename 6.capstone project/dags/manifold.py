...
with DAG(
    ...
    staging_quality_tasks: List[DataQualityCountOperator] = []
    ...
    for object_name, config in sql_queries_staging.copy_query_definition.items():
        ...
        data_quality: DataQualityCountOperator = DataQualityCountOperator(
            task_id= 'validate_' + object_name,
            dag=dag,
            postgres_conn_id='redshift_conn',
        ...
        staging_quality_tasks.append(data_quality)
    ...
    presentation_dim_quality_tasks: List[DataQualityDimensionOperator] = []
    ...
    for object_name, config in dimension_definitions.items():
        ...
        identification_columns: List[str] = config.get('identification_columns')
        ...
        data_quality_task: DataQualityDimensionOperator = DataQualityDimensionOperator(
            task_id = 'validate_' + object_name,
            dag=dag,
            postgres_conn_id='redshift_conn',
        ...
        presentation_dim_quality_tasks.append(data_quality_task)
        presentation_task >> data_quality_task
    ...
    staging_table_create_dummy >> staging_tasks
    staging_quality_tasks >> staging_table_populate_dummy

    # 5) populate the presentation Redshift layer's dimensions with SCD2
    staging_table_populate_dummy >> presentation_schema_creation >> presentation_dim_tasks 
    presentation_dim_quality_tasks >> presentation_dimension_dummy
